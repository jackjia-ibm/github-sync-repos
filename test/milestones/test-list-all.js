...
const debug = require('debug')('test:milestones:list-all');
...
describe('should be able to list milestones of repository', function() {
  it('should return label list without error', async function() {
    const result = await exec(prepareCliCommand(['milestones', 'list-all'], true, false, true));
    ...
    expect(result.stdout).to.match(new RegExp(`Total [0-9]+ milestone\\(s\\) in repository ${GITHUB_TEST_REPO1}:`));
    expect(result.stdout).to.match(new RegExp(`Total [0-9]+ milestone\\(s\\) in repository ${GITHUB_TEST_REPO2}:`));
    expect(result.stdout).to.match(new RegExp(`Total [0-9]+ milestone\\(s\\) in repository ${GITHUB_TEST_REPO3}:`));
  });
});
